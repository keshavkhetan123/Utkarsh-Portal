...
export default function JobOpeningGroupSelector(props: JobOpeningGroupsSelectorProps) {
  const { data: yearWisePrograms, isLoading } =
    api.placementConfig.getParticipatingGroupsForPlacementType.useQuery(
      props.jobTypeId,
    );
    ...
    <Paper className="flex flex-col gap-4 py-2 px-3">
      ...
      {!props.jobTypeId ? (
        ...
        <>
          ...
          <Button onClick={handleAddGroup} variant="outlined" className="mt-4 self-start">
            + Add Group
          </Button>
        </>
      )}
